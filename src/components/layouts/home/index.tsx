...
const Home = () => {
  ...
  const methods = {
    addToCart: (product: ProductModel) => {
      customDispatchEvent({name: 'ADD_PRODUCT_IN_CART', detail: { ...product, quantity: 1 }})
    },
  }
  ...
  return(
    <HomeContainer>
      ...
      {
        viewData?.content && viewData.content.map((content, index) => {
          ...
          switch(content.component) {
            ...
            case 'carrousel-home': {
              return <Carousel 
                key={`home_content_${index}`}
                items={content.items || []}
                itemsPerRow={(typeof content.itemsPerRow === 'string' ? Number.parseInt(content.itemsPerRow) : content.itemsPerRow) || 1}
                styles={{
                  margin: content.width === 100 ? '10px' : '10px auto',
                  padding: content.width === 100 ? '0' : '0 1rem',
                  maxWidth: content.width === 100 ? '100%' : `${content.width}rem`,
                }} />
            }
            case 'galeria-home': {
              return <Gallery 
      ...
      <ProductsList>
        {!loadingProducts && products && products.map((product: ProductModel) => (
          <ProductCard 
            key={product.id} 
            product={product} 
