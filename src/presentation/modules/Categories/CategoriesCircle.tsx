...
import { ArrowButtonCircle, ContainerSwiperCircle, ItemCircle, ItemContainerCircle, ItemImageCircle, ItemTitleCircle } from './CategoiresCircle.styles';
  ...
  return (
    <Fragment>
      <ContainerSwiperCircle>
        {props?.items?.length !== dymanicItemsPerRow && <ArrowButtonCircle onClick={() => swiper.slidePrev()} disabled={!isEnd}>
                ...
                <ItemCircle
                  ...
                  <Link href={getLink(item.link)}>
                    <ItemImageCircle
                      ...
                      alt={item.title}
                      style={{
                        width: dymanicItemsPerRow > limitItemBreakpoint ? '2.5rem' : '3rem',
                        height: dymanicItemsPerRow > limitItemBreakpoint ? '2.5rem' : '3rem'
                      }}
                    />
                  </Link>
                </ItemCircle>
                <ItemTitleCircle style={{ fontSize: dymanicItemsPerRow > limitItemBreakpoint ? '0.6rem' : '0.9rem' }}>{item.title}</ItemTitleCircle>
              </ItemContainerCircle>
            </SwiperSlide>
          ))}
          <div className='swiper-pagination-bullet custom-pagination-categories' />
        </Swiper>
        {props?.items?.length !== dymanicItemsPerRow && <ArrowButtonCircle onClick={() => swiper.slideNext()} disabled={isEnd}>
