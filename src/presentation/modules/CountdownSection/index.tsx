...
import { ProductSkuSellers, ProductSkuStruct } from '@/domain/entities/products/skus';
...
const CountdownSection = (props: CountdownStruct) => {
  ...
  const borderAssign = (index: number, quantity: number) => {
    ...
    if (quantity > 2) {
      switch (index) {
        ...
        case 3: return { borderLeft: borderColor };
        default:
  ...
  const handlePrices = (seller: ProductSkuSellers, type: string) => {
    ...
    switch (type) {
      ...
      case 'DISCOUNT':
        const discount = calculateDiscount(seller?.commertialOffer?.Price, seller?.commertialOffer?.ListPrice)
  ...
  return useMemo(() => (
    <React.Fragment>
      {checkActivation() && isEnabled && (
        <Container>
          <div style={{ maxHeight: 'max-content', width: '100%', margin: 'auto' }}>
            <Title text={title} />
            <CountdownSectionWrapper color={headerColor}>
              ...
              <CountdownContent>
                {products?.map((product: ProductSkuStruct, index: number) => (
                  <div key={index} style={{
                    display: 'flex', ...borderAssign(index + 1, products?.length),
                    width: '100%',
                    ...
                    <div>
                      <div style={{ minHeight: 200}}>
                        <div style={{ background: "#f2f2f2", borderRadius: 6, padding: '6px 10px', width: 'max-content', marginBottom: 8}}>
                          {product.brand}
                        </div>
                        <div>
                        ...
                        <div style={{ display: 'flex' }}>
                          <p style={{ fontSize: 20, fontWeight: 700 , marginBottom: 15}}>{handlePrices(product?.items[0]?.sellers[0], 'PRICE')}</p>
                          {handlePrices(product?.items[0]?.sellers[0], 'DISCOUNT') && <p
                            style={{
                              marginLeft: 10,
                              color: '#990707',
                              background: '#f1dddd',
                        ...
                        <p style={{ textDecoration: 'line-through', color: '#4d4d4d', fontSize: 12 }}>{handlePrices(product?.items[0]?.sellers[0], 'NORMAL')}</p>
                      </div>
                      <div style={{
  ...
  ), [checkActivation, title, headerColor, subtitle, endDate, products, isEnabled, backgroundColor]);
};
