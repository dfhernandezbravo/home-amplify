import { AdsCardBottom } from '@/presentation/components/atoms/AdsCardBottom';
...
const BottomCards = () => {
  ...
  return (
    <BottomCardsSection data-mobile={isSm}>
      {AdsCardsImages.map((card) => (
        <AdsCardBottom key={card.image} image={card.image} link={card.link} />
      ))}
    </BottomCardsSection>
  );
};
